...
Back into a dismissal. It wires mechanics only: whether the dialog may close,
whether the consumer vetoed, and whether a controlled dialog followed are all
the core's answers, read back as "is it still open".
...
| `guardBackNavigation(options)`        | Arms the history guard; returns the release.                           |
