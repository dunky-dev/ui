...
is one question: did this element actually render?

Two packages ask it for the same reason. `@dunky.dev/dom-focus-trap` filters
the Tab cycle, and `@dunky.dev/dom-overlay` filters the initial-focus
candidates; both are guarding against the same silent failure, so they must
