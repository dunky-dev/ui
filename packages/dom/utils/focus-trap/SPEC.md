...
| Rendered-ness via `Element.checkVisibility()`                   | The platform's own check, no bespoke style probing; jsdom lacks it, so the test setup (`vitest.setup.ts`) shims it for every suite. |
